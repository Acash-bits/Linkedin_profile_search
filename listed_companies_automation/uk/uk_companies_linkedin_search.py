...
def fetch_data(start_id):
    ...
    try:
        ...
        query = f"SELECT ID, company_name, Designation, Person_Name FROM UK_Key_people_info WHERE ID >= {start_id}"
...
def open_search_engine(driver, engine):
    ...
    try:
        ...
        time.sleep(3)  # Wait for page to load (this works for both Bing and Google)
...
def extract_linkedin_profile(driver):
    try:
        # Wait 3 seconds before extracting links to ensure page is fully loaded
        time.sleep(3)
...
def insert_result(conn, person_id, company_name, designation, linkedin_profile, person_name_from_db):
    try:
        cursor = conn.cursor()
        query = """
            INSERT INTO UK_LinkedIn_Profile (Person_ID, Company_Name, Designation, LinkedIn_Profile, Person_Name_DB)
            VALUES (%s, %s, %s, %s, %s)
...
def process_and_store(start_id):
    ...
    search_engines = ["bing", "google"]  # Now Bing comes first
